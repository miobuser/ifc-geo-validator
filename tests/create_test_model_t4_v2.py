"""
Create test model T4 (v2): L-shaped retaining wall as 2 separate elements.

Specifications:
- 2 separate IfcWall elements (PredefinedType=RETAININGWALL):
  1. Wall stem:  (0,0,0.5) to (6,0.3,3.0) — 6.0m x 0.3m x 2.5m
  2. Foundation:  (0,0,0)   to (6,2.0,0.5) — 6.0m x 2.0m x 0.5m
- Each wall has its own IfcFacetedBrep (6 quad faces per box)
- Both contained in one IfcFacility
- IFC4X3_ADD2 schema

Output: tests/test_models/T4_l_shaped.ifc
"""
...
def main():
    ...
    project.Description = "T4 — L-foermige Stuetzmauer (Mauersteg + Fundament, 2 Elemente)"

    # ---------- write file ----------
    out_dir = os.path.join(os.path.dirname(__file__), "test_models")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "T4_l_shaped.ifc")
    ifc.write(out_path)
    ...
    # ---------- verify ----------
    ifc2 = ifcopenshell.open(out_path)
    ifc_walls = ifc2.by_type("IfcWall")
    print(f"\nVerification: {len(ifc_walls)} IfcWall entities found")
    for w in ifc_walls:
        print(f"  - {w.Name} (PredefinedType={w.PredefinedType})")
    assert len(ifc_walls) == 2, f"Expected 2 IfcWall, got {len(ifc_walls)}"
    print("  OK — 2 IfcWall entities confirmed.")
