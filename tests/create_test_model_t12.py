"""
Create test model T12: Semicircular retaining wall (180° arc).

Specifications:
- 180° arc in plan view (half circle), 24 segments
- Inner radius R=6.0 m, wall thickness 0.3 m (outer R=6.3 m)
- Height: 3.0 m
- Crown with 3% transverse slope (inner edge z=3.0, outer edge z=3.009)
...
Output: tests/test_models/T12_semicircle.ifc
"""
...
def main():
    ...
    n_pts = N_SEG + 1  # 25 points for 24 segments
    ...
    # Project
    project = ifcopenshell.api.run("root.create_entity", ifc, ifc_class="IfcProject", name="Thesis Test T12")
    ...
    wall = ifcopenshell.api.run(
        ...
        ifc_class="IfcWall",
        name="Stützmauer T12 — Halbkreisförmige Wand (180° Bogen)",
    ...
    out_path = os.path.join(out_dir, "T12_semicircle.ifc")
    ifc.write(out_path)
    ...
    # V = (pi/2) * (R_outer^2 - R_inner^2) * H_avg
    h_avg = (Z_CROWN_INNER + Z_CROWN_OUTER) / 2.0
    v_approx = (math.pi / 2.0) * (R_OUTER**2 - R_INNER**2) * h_avg
